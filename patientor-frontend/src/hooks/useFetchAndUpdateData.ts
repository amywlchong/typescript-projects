...
const useFetchAndUpdateData = <T,>(serviceFunction: () => Promise<T[] | T | undefined>, defaultMessage: string, notFoundMessage: string) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const errorMessage = handleError(error, defaultMessage, notFoundMessage)
    ...
    fetchData().catch((error) => console.error('An error occurred while fetching data:', error));
  }, []);
  ...
  const updateItem = (updateFunction: ((prevData: T) => T)): void => {
    setData(prevData => {
      if (!Array.isArray(prevData) && prevData !== null) {
        ...
        console.warn('Attempted to use updateItem when data is an array or null.');
        return prevData;
      }
    });
  }
